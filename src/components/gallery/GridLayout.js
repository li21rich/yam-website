...
const WinnerBadge = ({ award }) => {
  const { color } = award;
  return (
    <div className="absolute top-2 left-2 bg-yellow-500 text-black rounded-full text-sm font-semibold shadow-md z-10 flex items-center justify-center w-8 h-8">
      <svg
        className="w-6 h-6"
        viewBox="0 0 20 20"
        fill={color}
